...
class PresensiController extends Controller
{
    ...
    public function show_presensiKosong($kelasId)
    {
        $kelas = Kelas::with(['siswa', 'walas'])->find($kelasId);

        if (!$kelas) {
            abort(404, 'Data kelas tidak ditemukan.');
        }

        $jadwalKBM = JadwalKBM::where('kelas_id', $kelasId)
            ->with(['mapel', 'guru'])
            ->orderByRaw("FIELD(hari, 'Senin', 'Selasa', 'Rabu', 'Kamis', 'Jumat', 'Sabtu', 'Minggu')")
            ->orderBy('mulai', 'asc')
            ->get();
        ...
        return view('pdf.presensi_kosong', [
            'kelas' => $kelas,
            'siswa' => $kelas->siswa,
            'walas' => $kelas->walas,
            'jadwalPerHari' => $jadwalPerHari,
    ...
    public function show_rekap(Request $request, $id)
    {
        if (!$request->filled('dari')) {
            return response()->json([
                'success' => false,
